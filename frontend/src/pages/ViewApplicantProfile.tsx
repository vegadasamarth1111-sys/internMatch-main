...
export function ViewApplicantProfile() {
  ...
  const getInitials = () => {
    if (!profile) return 'U'
    return ((profile.first_name?.charAt(0) ?? '') + (profile.last_name?.charAt(0) ?? '')).toUpperCase() || 'A'
  }
      ...
      <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white rounded-lg shadow-md p-12 text-center">
            <DefaultAvatar size="lg" initials="A" />
            <h2 className="text-xl font-semibold text-gray-700 mt-4">Profile not complete</h2>
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-md p-8">
          ...
          {(profile.education_level || profile.degree_name || profile.university_name) && (
            <div className="mb-8">
              <h2 className="text-xl font-bold text-gray-900 mb-3">Education</h2>
              <p className="text-gray-700">{[profile.degree_name, profile.university_name].filter(Boolean).join(' - ')}</p>
